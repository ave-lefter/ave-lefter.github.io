...
function listMapFunction(i:Record<string,any>) {
  ...
  time_arr.forEach((t) => {
    (progress_obj[`progress_buys_tx_${t}_count`] =
      i[`sells_tx_${t}_count`] + i[`buys_tx_${t}_count`] > 0
        ? (i[`buys_tx_${t}_count`] /
            (i[`sells_tx_${t}_count`] + i[`buys_tx_${t}_count`])) *
          100
        : 0),
      (progress_obj[`progress_sells_tx_${t}_count`] =
        i[`sells_tx_${t}_count`] + i[`buys_tx_${t}_count`] > 0
          ? (i[`sells_tx_${t}_count`] /
              (i[`sells_tx_${t}_count`] + i[`buys_tx_${t}_count`])) *
            100
          : 0),
      (progress_obj[`progress_buy_volume_u_${t}`] =
        i[`buy_volume_u_${t}`] + i[`sell_volume_u_${t}`] > 0
          ? (i[`buy_volume_u_${t}`] /
              (i[`buy_volume_u_${t}`] + i[`sell_volume_u_${t}`])) *
            100
          : 0),
  ...
  return {
    ...
    lp_locked_percent:
      Number(i.lp_locked_percent * 100 || 0) < 1
        ? 0
        : Number(i.lp_locked_percent * 100 || 0),
    lp_locked_to:
      i?.lp_locked_to !== '1970-01-01T00:00:00Z' &&
      i?.lp_locked_to !== '0001-01-01T00:00:00Z'
