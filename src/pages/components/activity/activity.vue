...
const pageInfo = ref({
  pageNO: useSessionStorage('activity-pageNO', 1).value,
  pageSize: 50,
  total: 0,
})
watch(
  ...
  (val) => {
    useSessionStorage('activity-pageNO', 1).value = val
...
onActivated(() => {
  ...
  pageInfo.value.pageNO = useSessionStorage('activity-pageNO', 1).value
  initCache()
  clearTimeout(timer)
