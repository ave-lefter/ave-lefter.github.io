...
<template>
  <el-table-column
    :label="$t('security')"
    :width="getTextWidth($t('security'), 50) + 35"
    align="right"
  >
      ...
      <el-popover :width="247">
        <template #reference>
          <div
            ...
            :class="
              row.rug_rate > 60 ? 'color-#F6465D' : 'color-[--d-999-l-666]'
            "
          >
        ...
        <template #default>
          <el-row align="middle" class="mb-16px">
            <el-col :span="24">
              <div class="lh-20px">
                ...
                <span
                  >{{
                    formateMin(row.all_tag_rate) ||
                    formatNumber(row.all_tag_rate || 0, 1)
                  }}%</span
                >
              </div>
            </el-col>
          ...
          <template
            v-for="(item, index) in getProgressData(row)"
