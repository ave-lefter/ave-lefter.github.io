...
import { type KlineInfo } from '@/api/perp/index'
...
export function switchPerpResolution(resolution: string) {
  const obj: Record<string, string> = {
    '1': 'MINUTE_1',
    '5': 'MINUTE_5',
    '15': 'MINUTE_15',
...
export function updatePerpLastBar(
  wsData: KlineInfo,
  contractId: string,
  lastBar: KLineBar | null,
  intervalInSeconds: number | string
): KLineBar | null {
  const address = contractId.toLowerCase()
  const txTimeMs = wsData.time
  const interval =
    typeof intervalInSeconds === 'number' ? intervalInSeconds : Number(intervalInSeconds)
  ...
  if (!lastBar) {
    return {
      time: barStartTime,
      open: Number(wsData.open),
      high: Number(wsData.open),
      low: Number(wsData.open),
      close: Number(wsData.open),
      volume: Number(wsData.volume),
    }
  }
