    ...
    <div class="m-table w-100% mt-12px flex-1 overflow-hidden bg-[--secondary-bg]">
      ...
      :default-sort="defaultSort" @sort-change="handleSortChange" @row-click="tableRowClick"  @selection-change="_handleSelectionChange" :row-key="(row:any)=>`${row?.user_address}-${row?.user_chain}`">
        <template #empty>
          ...
          <div v-else-if="!loading && followStore.currentAddress" class="flex flex-col items-center justify-center py-30px">
...
<!-- last_tx_time -->
      ...
      <el-table-column v-if="!isMonitor" :label="t('addrGroup')" align="right" width="160px">
        <template #default="{ row }">
          <el-select v-model="row.group_id" class="[&&]:[--el-text-color-regular:var(--secondary-text)] [&&]:[--el-input-icon-color:var(--secondary-text)] [&&]:[--el-select-width:100px]" popper-class="w-193px w-addrGroup" filterable :persistent="false" @click.stop @change="(val) => getRowGroupChange(val, row)">
...
const { addressGroups ,currentAddress,updateNum12,updateNum13,updateNum2,updateNum3,addressConditions,addressConditions2} = storeToRefs(useFollowStore())
...
const conditions=computed(() => {
  return props.isMonitor?addressConditions2.value:addressConditions.value
})
...
const pageData=computed(() => {
  return props.isMonitor?pageData2.value:pageData1.value
})
...
const filterDataSource=computed(() => {
  return props.isMonitor?dataSource2.value:dataSource.value
})
...
const _handleSelectionChange = (val: any[]) => {
  props.isMonitor?handleSelectionChange2(val):handleSelectionChange(val)
}
...
watch([() => conditions.value, ()=>updateNum2.value+updateNum3.value+(!props.isMonitor?updateNum13.value:updateNum12.value),()=>props.isMonitor], (value, oldValue) => {
  init()
...
const handleMonitor = throttle((row:any,index:number=0) => {
  ...
  if(props.isMonitor||(!props.isMonitor&&(row.is_monitored === 1))) {
...
const getTableList = throttle(function() {
  ...
  const req=props.isMonitor?monitorAddresses: getAttentionPageList
  ...
  req({...conditionsData, pageNO: pageData.value.page, pageSize: pageData.value.pageSize, ...last_trade_time}).then((res) => {
    console.log('getAttentionPageList res',props.isMonitor, res)
    const tableData =props.isMonitor?dataSource2:dataSource
