...
watch(()=>wsPrivateStore.wsResult,(val)=>{
  listData.value = val['trade-event']?.content?.data?.position || []
},{immediate:true})
watch(()=>wsPublicStore.wsResult[WSPerpEventType.TICKER_ALL_1S],(val)=>{
  listData.value.forEach(el=>{
      ...
      if(updateData){
        ...
        el.unrealizedPnl = updateData.oraclePrice * el.openSize - el.openValue
        el.unrealizedPnlRate = el.unrealizedPnl / el.openValue * 1000
      }
    })
})
watch(()=>perpStore.totalAssets?.accountList,(val)=>{
  console.log(val?.[0]?.positionAssetList,listData.value)
...
<template>
  <el-table fit :data="listData" header-row-class-name="text-12px sticky top-0 z-10 font-500" cell-class-name="color-[--secondary-text] text-12px " row-class-name="cursor-pointer">
      <el-table-column :label="t('perp')" prop="perp" >
        <template #default="{ row }">
          ...
          <div class="text-10px lh-14px">
            做多 {{row.maxLeverage}}x
          </div>
        </template>
      </el-table-column>
      ...
      <el-table-column align="right" :label="t('unrealizedPnl')" prop="unrealizedPnl" >
        <template #default="{ row }">
          <div class="text-12px lh-18px flex flex-col gap-4px">
            <span :class="getColorClass(row.unrealizedPnl)">{{addSign(row.unrealizedPnl)}}{{formatNumber(Math.abs(row.unrealizedPnl),2)}}</span>
            <span :class="getColorClass(row.unrealizedPnlRate)">({{addSign(row.unrealizedPnlRate)}}{{ formatNumber(Math.abs(row.unrealizedPnlRate),2) }}%)</span>
          </div>
        </template>
      </el-table-column>
