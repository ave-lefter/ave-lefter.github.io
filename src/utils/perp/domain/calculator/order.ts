...
import type { AccountInfo, IContract, IMetadata, OrderEntry, PositionEntry } from "../../types";
...
export const calculateOrderFillData = ({
  ...
  let orderLeftSize = BigNumber(0);
  if (["PENDING", "OPEN", "CANCELING"].includes(order.status)) {
    orderLeftSize = BigNumber(order.size || 0).minus(order.cumFailSize || 0).minus(order.cumFillSize || 0);
  } else {
    orderLeftSize = BigNumber(order.cumMatchSize || 0).minus(order.cumFailSize || 0).minus(order.cumFillSize || 0);
  }
