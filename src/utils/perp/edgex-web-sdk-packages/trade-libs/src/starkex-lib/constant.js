...
import { mapValues, without } from "@edgex/utils";
import BN from "bn.js";
import { keccak256 } from "ethereum-cryptography/keccak";
import { _normalizeHex32 } from "./lib/util";
...
export const SYNTHETIC_ASSETS = without(ALL_ASSETS, COLLATERAL_ASSET);
...
export const COLLATERAL_ASSET_ID_BY_NETWORK_ID = (token) => {
  const metadata = window.__FUTURES_METADATA__;
 ...
 */
export const SYNTHETIC_ASSET_ID_MAP = (() => {
  const result = {};
  SYNTHETIC_ASSETS.forEach((asset) => {
    result[asset] = makeSyntheticAssetId(asset);
  });
  return result;
})();
 ...
 */
export const ASSET_QUANTUM_SIZE = mapValues(ASSET_RESOLUTION, (resolution) => `1e-${resolution}`);
...
function makeCollateralAssetId(tokenAddress, quantization = 1) {
  const data = Buffer.concat([
    ...
    Buffer.from(_normalizeHex32(tokenAddress), "hex"),
    Buffer.from(_normalizeHex32(new BN(quantization).toString(16)), "hex"),
  ]);
  const result = keccak256(data);
  const resultBN = new BN(result.toString("hex"), 16);
  resultBN.imaskn(250);
  return `0x${_normalizeHex32(resultBN.toString(16))}`;
}
