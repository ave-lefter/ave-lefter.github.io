import { _asEcKeyPair, asEcKeyPairPublic, asSimpleSignature, deserializeSignature, serializeSignature } from "../helpers";
...
export class StarkSignable {
  ...
  async sign(privateKey) {
    ...
    const ecSignature = await sign(_asEcKeyPair(privateKey), hashBN);
  ...
  async verifySignature(signature, publicKey, publicKeyYCoordinate = null) {
    ...
    const hashBN = await this.getHashBN();
    return (await verify(asEcKeyPairPublic(publicKey, false), hashBN, signatureStruct)) || verify(asEcKeyPairPublic(publicKey, true), hashBN, signatureStruct);
  }
}
