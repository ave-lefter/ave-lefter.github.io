...
import { bnToHex32, _normalizeHex32 } from "../lib/util";
 ...
 */
export function _asEcKeyPair(privateKeyOrKeyPair) {
  const privateKey =
    typeof privateKeyOrKeyPair === "string" ? privateKeyOrKeyPair : privateKeyOrKeyPair.privateKey;
  return starkEc.keyFromPrivate(_normalizeHex32(privateKey));
}
/**
...
export function asEcKeyPairPublic(publicKey, yCoordinateIsOdd = null) {
  if (typeof publicKey !== "string") {
    ...
    return starkEc.keyFromPublic({
      x: _normalizeHex32(publicKey.x),
      y: _normalizeHex32(publicKey.y),
    });
  }
  ...
  const prefixedPublicKey = `${prefix}${_normalizeHex32(publicKey)}`;
 ...
 */
export function _asSimpleKeyPair(ecKeyPair) {
...
export function isValidPublicKey(publicKey) {
  try {
    ...
    if (_normalizeHex32(resultX) !== _normalizeHex32(expectedX)) {
      return false;
    }
    return true;
  } catch (error) {
...
export function serializeSignature(signature) {
  return `${_normalizeHex32(signature.r)}${_normalizeHex32(signature.s)}`;
}
/**
