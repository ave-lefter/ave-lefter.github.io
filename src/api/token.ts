...
export const bot_getUserWalletTxInfo = createCacheRequest(async function(query: {
    ...
    return getTonTokenList(query.user_address, query.user_token).then(async res => {
      ...
      return getTokenPnl({
        ...
        return [{
          ...
          unrealized_profit: Number(item?.balance) > 0 ? res?.profitUnrealized || '0' : '0',
          unrealized_ratio: Number(item?.balance) > 0 ? res?.unrealizedRatio || '0' : '0',
  ...
  return getBalances({
    ...
    return res.tokens.map(item => ({
      ...
      unrealized_profit: Number(item?.balance) > 0 ? item.pnl.unrealizedProfit : 0,
      unrealized_ratio: Number(item?.balance) > 0 ? item.pnl.unrealizedProfitRatio : '0',
      ...
      average_purchase_price_usd: item.pnl.averageNetPurchasePrice,
