...
export async function quoteBestRouterV2({from_token, to_token, amountIn, amountOut}: {from_token: string, to_token: string, amountIn: string, amountOut: string}, wrapper: string, chain = useWalletStore().chain, max_hops = 3, max_routes = 6) {
  ...
  from_token = from_token === NATIVE_TOKEN ? (chain === 'anubis' ? '0x83fd06F0846d9D90B3016bF670Efe2E0B11cDe14' : from_token) : from_token
  to_token = to_token === NATIVE_TOKEN ? (chain === 'anubis' ? '0x83fd06F0846d9D90B3016bF670Efe2E0B11cDe14' : to_token) : to_token
