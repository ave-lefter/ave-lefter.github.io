 ...
 <el-scrollbar v-else height="calc(100% - 120px)" @end-reached="onScrollEnd">
  <div class="flex flex-col gap-16px">
    <div v-for="(item, index) in trackerStore.list" :key="item.id" class="border-b-1px border-b-solid border-b-[--border]">
      ...
      <div v-if="['2','3','4'].includes(item.type) && (item.retweeted_tweet||item.quoted_tweet||item.replied_tweet)" class="border-1px border-solid border-[--dialog-divider] rounded-8px px-12px pt-16px ml-40px mb-16px">
