<template>
  <div class="bg-[--main-bg] rounded-8px p-12px min-w-[360px]">
        ...
        <div class="flex items-center justify-between">
          <div class="flex items-center gap-4px">
            <svg width="18" height="18" viewBox="0 0 16 16" fill="none" xmlns="http://www.w3.org/2000/svg">
              <path d="M5.59998 3.8H7.39998V9.2H5.59998V11H4.39998V9.2H2.59998V3.8H4.39998V2H5.59998V3.8ZM11.6 6.8H13.4V12.2H11.6V14H10.4V12.2H8.59998V6.8H10.4V5H11.6V6.8Z" fill="var(--main-text)"/>
            </svg>
            <span class="text-18px font-600 color-[--main-text]">
      ...
      <div class="flex items-center justify-between mb-8px text-12px">
        ...
        <div class="flex items-center gap-4px">
          <span class="color-[--main-text] font-600">
            {{ currentTimeData ? formatNumberS((parseFloat(currentTimeData.buy_volume || '0') + parseFloat(currentTimeData.sell_volume || '0')).toString()) : '$0' }}
          </span>
          <span
            v-if="currentTimeData"
            class="text-12px"
          ...
          >
            {{ getChangeText(getAverageChange(currentTimeData.buy_volume_period || '0', currentTimeData.sell_volume_period || '0')) }}
          </span>
        </div>
      </div>
      <div class="progress-bar relative h-8px rounded-4px overflow-hidden bg-[--main-bg]">
        <div class="absolute left-0 top-0 h-full bg-[--up-color]" :style="`width: ${getBuySellRatio().buyWidth}%`"></div>
        <div class="absolute right-0 top-0 h-full bg-[--down-color]" :style="`width: ${getBuySellRatio().sellWidth}%`"></div>
      </div>
      <div class="flex items-center justify-between mt-4px text-12px">
        <span class="color-[--up-color]">
          {{ currentTimeData ? formatNumberS(currentTimeData.buy_cnt || '0') + ' / ' + formatNumberS(currentTimeData.buy_volume || '0', 2) : '0 / $0' }}
        </span>
        <span class="color-[--down-color]">
          {{ currentTimeData ? formatNumberS(currentTimeData.sell_cnt || '0') + ' / ' + formatNumberS(currentTimeData.sell_volume || '0', 2) : '0 / $0' }}
        </span>
      </div>
    </div>
    ...
    <div class="token-stats mb-14px">
      <div class="flex items-center gap-4px mb-4px">
        <svg width="16" height="16" viewBox="0 0 16 16" fill="none" xmlns="http://www.w3.org/2000/svg">
          <path d="M14.5332 10.1338C14.5332 12.1221 11.5785 13.7333 7.93359 13.7334C4.35354 13.7334 1.43876 12.1793 1.33594 10.2402L1.33301 10.1338V8.93359C1.33301 10.922 4.28859 12.5342 7.93359 12.5342C11.5785 12.5341 14.5332 10.9219 14.5332 8.93359V10.1338ZM14.5996 8C14.5996 9.98839 11.645 11.6006 8 11.6006C4.4198 11.6006 1.50592 10.0456 1.40332 8.10645L1.40039 8V6.80078C1.40071 8.78903 4.3552 10.4004 8 10.4004C11.6448 10.4004 14.5993 8.78903 14.5996 6.80078V8ZM8 2C11.645 2.00001 14.5996 3.61219 14.5996 5.60059C14.5995 7.58892 11.6449 9.20018 8 9.2002C4.35508 9.2002 1.40052 7.58893 1.40039 5.60059C1.40039 3.61219 4.355 2 8 2Z" fill="var(--main-text)"/>
        </svg>
    ...
    <div class="top-platforms">
      ...
      <div class="flex items-center gap-8px">
        <div v-for="platform in platforms" :key="platform.name" class="flex gap-8px w-[130px] border border-solid border-[var(--border)] rounded-8px px-6px py-8px">
          <div class="flex items-center">
            <div class="w-34px h-34px rounded-full bg-[--main-bg] flex-center" :style="{ border: `1px solid ${platform.color}` }">
...
const updateCurrentTimeData = () => {
  ...
  const chain = walletStore.chain || 'solana' // 默认使用 solana
  // 根据链类型确定平台标识
  const platform = chain === 'bsc' ? 'bsc_fourmemev2' : 'solana_pump'
