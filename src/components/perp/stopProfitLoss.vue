  ...
  <el-dialog v-model="visible" append-to-body :title="t('takeProfitStopLoss2')" width="500px" destroy-on-close>
    <div class="overflow-y-auto scrollbar-hide mx--16px px-16px">
      ...
      <div>
        ...
        <div class="flex">
          <el-button class="h-40px flex-1 m-l-auto" :color="themeStore.isDark ? '#333' : '#F2F2F2'" @click="visible=false">
            {{ $t('cancel') }}
          </el-button>
          <el-button type="primary" class="h-40px flex-1 m-l-auto" :disabled="disabled" :loading="loading" @click="_createOrder">
            {{ $t('confirm') }}
          </el-button>
        </div>
      </div>
