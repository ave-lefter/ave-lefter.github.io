<template>
  <div class="w-position bg-[--secondary-bg] w-100% h-100% pl-12px pr-12px relative overflow-hidden">
    ...
    <div
      ...
      <span class="color-[--main-text] text-14px">{{ $t('favorites') }}</span>
    ...
    <div v-if="botStore.evmAddress || walletStore.address" class="w-full relative">
      <FavoriteTable :height="scrollHeight" class="absolute w-[calc(100%+21px)] left--10px" />
    </div>
    <AveEmpty
      v-else
...
import FavoriteTable from '@/pages/token/components/left/favoriteTable.vue'
const { t } = useI18n()
const botStore = useBotStore()
const walletStore = useWalletStore()
const { isLeftFixed, isRightFixed } = storeToRefs(useFavTokenStore())
...
const positionStore = useFavTokenStore()
