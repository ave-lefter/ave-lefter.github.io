    ...
    <div :style="botStore.connectVisible&&[{filter: 'url(#blur)'}]" class="w-100vw h-100vh">
      <TheHeader/>
       <!-- :style="signalStore.translateStyle"  translate-x-0px-->
        <TokenHistory v-if="tokenHistoryVisible && globalStore.tokenHistoryVisible && route.name !=='perp-id'" class="border-t-solid border-t-[--main-divider]" :style="{paddingLeft:_style.paddingLeft,paddingRight:_style.paddingRight}"/>
      <div
...
import { useEventBus } from '@vueuse/core'
  ...
  const TrackerDragger = defineAsyncComponent(()=>import('~/components/tracker/index.vue'))
